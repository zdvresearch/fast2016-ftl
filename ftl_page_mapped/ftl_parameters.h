...
#define MaxLowPage                      (125)
#define UsedPagesPerRecycledLogBlk      (31)
...
#define CHUNKS_PER_RECYCLED_PAGE            4  // 4KB chunks
#define CHUNKS_PER_BLK                      (CHUNKS_PER_PAGE * PAGES_PER_BLK)
#define CHUNKS_PER_LOG_BLK                  (UsedPagesPerLogBlk * CHUNKS_PER_PAGE) // 126 pages are used because the last low-high couple is used for lpns lists
//
/* The max number of valid pages for the first write is 126 because
 * the last low page is used for the current lpns list, and the last
 * high page will be used by the lpns list of the second usage.
 * The number of valid pages will be used to initialize the heap when
 * starting the second usage in log.c.
 * The last low page will be reused, all chunks will be seen invalid
 * (no data is written there) therefore the heap entry will be increased
 * by 8. This is the reason why we start the first usage with 126 page instead of 127.
 */
#define CHUNKS_PER_LOG_BLK_FIRST_USAGE      ( ((PAGES_PER_BLK)-2) * CHUNKS_PER_PAGE)
#define CHUNKS_PER_LOG_BLK_SECOND_USAGE      ( ((PAGES_PER_BLK)-1) * CHUNKS_PER_PAGE) // 127 pages are used because the last high page is used for lpns list.
                                                                                      // The last low page was already used for the first lpns list, but it will be overwritten and the number of valid chunks will be decreased accordingly.
//#define CHUNKS_PER_LOG_BLK_FIRST_USAGE      CHUNKS_PER_LOG_BLK_SECOND_USAGE
#define CHUNKS_PER_RECYCLED_LOG_BLK         (UsedPagesPerRecycledLogBlk * CHUNKS_PER_RECYCLED_PAGE)
...
#define SECTORS_PER_ENCODED_CHUNK           (21)
...
//#define SPARE_LOG_BLK_PER_BANK  ((DATA_BLK_PER_BANK*7)/100)
...
#define LOG_BLK_PER_BANK            (DATA_BLK_PER_BANK)

#if LOG_BLK_PER_BANK > VBLKS_PER_BANK
    #define LOG_BLK_PER_BANK            VBLKS_PER_BANK
#endif

#define MaxRecycledBlocks       (LOG_BLK_PER_BANK / 30)
...
#define FTL_BMT_BYTES       ((DATA_BMT_BYTES + LOG_BMT_BYTES + BYTES_PER_SECTOR - 1) / BYTES_PER_SECTOR * BYTES_PER_SECTOR)
...
#define NUM_GC_BUFFERS          (NUM_BANKS)
...
#define VICTIM_LPN_LIST_BYTES               (NUM_BANKS * BYTES_PER_PAGE)                                                                                // 2 MB
...
#define RECYCLED_CLEAN_LIST_NODES_BYTES     ((NUM_BANKS * MaxRecycledBlocks * sizeof(logListNode) + DRAM_ECC_UNIT -1) / DRAM_ECC_UNIT * DRAM_ECC_UNIT)     // 2,560 B
#define PRECACHE_FOR_ENCODING_BYTES         (NUM_BANKS * BYTES_PER_PAGE)
#if Overwrite
...
#define DramLogBufLpn        ((LOG_BLK_PER_BANK)*(PAGES_PER_BLK)-1)
//#define ColdLogBufLpn       (HotLogBufLpn + 1)
#define ColdLogBufBitFlag   (1 << 31)
...
//
typedef enum {Invalid, FlashWLog, FlashWLogEncoded, DRAMHotLog, DRAMColdLog} chunkLocation;
