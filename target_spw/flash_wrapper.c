...
#include "jasmine.h"
#include "ftl_metadata.h" // userSecWrites, totSecWrite
...
void nand_page_ptread_to_host(UINT32 const bank, UINT32 const vblock, UINT32 const page_num, UINT32 const sect_offset, UINT32 const num_sectors)
{
    ...
    uart_print("nand_page_ptread_to_host bank="); uart_print_int(bank);
    uart_print(", vblock="); uart_print_int(vblock);
    uart_print(", page="); uart_print_int(page_num);
    uart_print(", sect_offset="); uart_print_int(sect_offset);
...
void nand_page_ptprogram(UINT32 const bank, UINT32 const vblock, UINT32 const page_num, UINT32 const sect_offset, UINT32 const num_sectors, UINT32 const buf_addr, UINT32 const issue_flag)
{
    ...
    row = (vblock * PAGES_PER_BLK) + page_num;
    uart_print("nand_page_ptprogram bank="); uart_print_int(bank);
    uart_print(", vblock="); uart_print_int(vblock);
    uart_print(", page="); uart_print_int(page_num);
    uart_print(", sect_offset="); uart_print_int(sect_offset);
    uart_print(", num_sectors="); uart_print_int(num_sectors); uart_print("\r\n");
...
void nand_page_program_from_host(UINT32 const bank, UINT32 const vblock, UINT32 const page_num)
{
    ...
    uart_print("nand_page_program_from_host bank="); uart_print_int(bank);
    uart_print(", vblock="); uart_print_int(vblock);
    uart_print(", page="); uart_print_int(page_num); uart_print("\r\n");
    uart_print("Writing row="); uart_print_int(row); uart_print("\r\n");
...
void nand_page_copyback(UINT32 const bank, UINT32 const src_vblock, UINT32 const src_page, UINT32 const dst_vblock, UINT32 const dst_page){
    uart_print("nand_page_copyback(bank="); uart_print_int(bank);
    uart_print(", src_vblock="); uart_print_int(src_vblock);
    uart_print(", src_page="); uart_print_int(src_page);
    uart_print(", dst_vblock="); uart_print_int(dst_vblock);
    uart_print(", dst_page="); uart_print_int(dst_page); uart_print("\r\n");
    BOOL32    do_copyback;
    ...
    if (do_copyback)
    {
        uart_print("Do internal copyback\r\n");
    ...
    {
        uart_print("Cannot do internal copyback\r\n");
        SETREG(FCP_CMD, FC_COL_ROW_READ_OUT);
        SETREG(FCP_OPTION, FO_P | FO_E);
        ...
        FLASH_POLL_WR("Warning1 in nand_page_copyback\r\n")
        FLASH_ISSUE;
        ...
        SETREG(FCP_BANK, REAL_BANK(bank));
        FLASH_POLL_WR("Warning2 in nand_page_copyback\r\n")
        FLASH_ISSUE;
    }

}
// This function copyback a page modifiying a part of it with the content given in a buffer
// Parameters:
// sect_offset: sector in the flash page where to insert the modified buffer
// dma_addr: start address of the modified buffer
// dma_count: bytes of the modified buffer
...
{
    uart_print("nand_block_erase(bank=");
    uart_print_int(bank);
    uart_print(", vblock=");
    uart_print_int(vblock);
    uart_print(")\r\n");
    ASSERT(bank < NUM_BANKS);
    ASSERT(vblock < VBLKS_PER_BANK);
