...
#endif
{
    ...
    else if (intr_stat & INTR_SDRAM)
    {
        ...
        if (sdram_interrupt & SDRAM_INT_ECC_CORR)
        {
            ...
            uart_print("INTERRUPT in misc.c: SDRAM ECC Correct\r\n");
            g_sdram_ecc_count++;
        }
        ...
        if (sdram_interrupt & SDRAM_INT_ECC_FAIL)
        {
            ...
            uart_print_level_1("INTERRUPT in misc.c: SDRAM ECC Fail\r\n");
            while(1);
        ...
        if (sdram_interrupt & SDRAM_INT_ADDR_OF)
        {
            ...
            uart_print_level_1("INTERRUPT in misc.c: SDRAM interrupt occurred: attempt to access beyond DRAM address boundary");
            while(1);
        }
    }
}
    ...
    for (vblk_offset = 1; vblk_offset < VBLKS_PER_BANK; vblk_offset++)
    {
        ...
        for (bank = 0; bank < NUM_BANKS; bank++)
        {
            ...
            while ((GETREG(WR_STAT) & 0x00000001) != 0) {
                count++;
                if (count > 100000) {
                    uart_print_level_1("warning1 in test_nand_blocks\r\n");
                    count=0;
                }
            }
        ...
        for (page_offset = 0; page_offset < PAGES_PER_VBLK; page_offset++)
        {
            for (bank = 0; bank < NUM_BANKS; bank++)
            {
                ...
                while ((GETREG(WR_STAT) & 0x00000001) != 0) {
                    count++;
                    if (count > 100000) {
                        uart_print_level_1("warning2 in test_nand_blocks\r\n");
                        count=0;
                    }
                }
                SETREG(FCP_ISSUE, NULL);
        ...
        while (GETREG(MON_CHABANKIDLE) != 0) {
            count++;
            if (count > 100000) {
                uart_print_level_1("warning3 in test_nand_blocks\r\n");
                count=0;
            }
        }
        ...
        for (page_offset = 0; page_offset < PAGES_PER_VBLK; page_offset++)
        {
            for (bank = 0; bank < NUM_BANKS; bank++)
            {
                ...
                while ((GETREG(WR_STAT) & 0x00000001) != 0) {
                    count++;
                    if (count > 100000) {
                        uart_print_level_1("warning4 in test_nand_blocks\r\n");
                        count=0;
                    }
                }
                SETREG(FCP_ISSUE, NULL);
        ...
        while (GETREG(MON_CHABANKIDLE) != 0) {
            count5++;
            if (count5 > 100000) {
                uart_print_level_1("warning5 in test_nand_blocks\r\n");
                count=0;
            }
        }
        ...
        for (bank = 0; bank < NUM_BANKS; bank++)
        {
            ...
            while ((GETREG(WR_STAT) & 0x00000001) != 0) {
                count++;
                if (count > 100000) {
                    uart_print_level_1("warning6 in test_nand_blocks\r\n");
                    count=0;
                }
            }
            SETREG(FCP_ISSUE, NULL);
