...
ATA_FUNCTION_T search_ata_function(UINT32 command_code)
{
    UINT32 index;
    ...
    if (ata_function == (ATA_FUNCTION_T) INVALID32) {
        uart_print_level_1("Warning: ATA function lookup returned INVALID! ");
        uart_print_level_1("Function index was "); uart_print_level_1_int(index); uart_print_level_1("\r\n");
        ...
        if (eventq_get_count())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //backgroundCleaning(bankForBGCleaning);
                    //bankForBGCleaning = (bankForBGCleaning+4) % NUM_BANKS;
                }
            }
        }
        else
