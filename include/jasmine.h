...
#define DRAM_SIZE       65075200 // 64MB
//#define BANK_BMP        0x0001 // 1 bank
//#define BANK_BMP        0x0101 // 2 banks
//#define BANK_BMP        0x0011 // 2 banks
//#define BANK_BMP        0x1111 // 4 banks
//#define BANK_BMP        0x00030003 // 4 banks
//#define BANK_BMP        0x000C000C // 4 banks
#define BANK_BMP        0x0F0F0F0F // 16 banks
//#define BANK_BMP        0x000F // 16 banks
//#define BANK_BMP        0xFFFFFFFF // 32 banks
#define NUM_CHANNELS    4

#define CLOCK_SPEED     175000000

#define OPTION_2_PLANE                  1    // 1 = 2-plane mode, 0 = 1-plane mode
#define WOMCanFail                      0   // 1 = WOM can fail with rate 100 - successRateWOM, 0 = WOM always succeeds
                                            // Warning: this option involves floating point calculation and inclusion of the std lib
#define CanReuseBlksForColdData         0
#define OPTION_NO_DRAM_ABSORB           1   // 1 = no DRAM absorb
...
#define DetailedOwStats                 0    // 1 = print when pages are actually overwritten and swaps, 0 = disable
#define MeasureDetailedOverwrite        0    // 1 = print GC time measurements on UART, 0 = disable
#define MeasureGc                       1    // 1 = print GC time measurements on UART, 0 = disable
#define MeasureW                        0    // 1 = print GC time measurements on UART, 0 = disable
#define MeasureOwEfficiency             0 // 1 = count how many sectors are written to ow blocks before erasing, 0 = disable
#define MeasureDramAbsorb               0   // 1 = print when sectors are absorbed by DRAM, 0 = disable
#define Overwrite                       0   // 1 = the FTL supports overwrites, 0= disable
#define PrintStats                      0
#define OPTION_UART_DEBUG               0   // 1 = enable UART message output, 0 = disable
#define OPTION_DEBUG_HEAP               0
#define OPTION_DEBUG_DRAM               0
#define OPTION_DEBUG_WRITE              0
#define OPTION_DEBUG_GARBAGE_COLLECTION 0
#define OPTION_UART_DEBUG_DRAM          0   // 1 = enable UART message output, 0 = disable
#define OPTION_UART_DEBUG_HEAP          0   // 1 = enable UART message output, 0 = disable
#define OPTION_UART_DEBUG_LIST          0   // 1 = enable UART message output, 0 = disable
#define OPTION_DEBUG_HASH               0
#define NO_FLASH_OPS                    0 // 1 = no flash ops are performed, used to test metadata management
#define OPTION_HYBRID                   0  // 0 - Page Mapped, 1 - Hybrid
#define OPTION_OUT_OF_ORDER_WRITES      0 // 1 = enable writes out of order, 0 = disable
...
//-------------------------------------------------------------
// This parameter controls DataBlks / TotalBlks ratio
//-------------------------------------------------------------
//#define NUM_LSECTORS        (NUM_PSECTORS)              // 2048 data blk per bank, 128 real blocks if overwriting
//#define NUM_LSECTORS      (NUM_PSECTORS  / 2)         // 1024 data blk per bank, 128 real blocks if overwriting
#define NUM_LSECTORS      (NUM_PSECTORS  / 4)         // 512 data blk per bank, 128 real blocks if overwriting
//#define NUM_LSECTORS    (NUM_PSECTORS  / 8)       // 256 data blk per bank, 128 real blocks if overwriting
//#define NUM_LSECTORS    (NUM_PSECTORS  / 16)      // 128 data blk per bank, 64 real blocks if overwriting
//#define NUM_LSECTORS    (NUM_PSECTORS  / 32)      // 64 data blk per bank, 32 real blocks if overwriting
//#define NUM_LSECTORS    (NUM_PSECTORS  / 64)      // 32 data blk per bank, 16 real blocks if overwriting
//#define NUM_LSECTORS    (NUM_PSECTORS  / 128)     // 16 data blk per bank, 8 real blocks if overwriting
//#define NUM_LSECTORS    (NUM_PSECTORS  / 256)     // 8 data blk per bank, 4 real blocks if overwriting
//#define NUM_LSECTORS    (NUM_PSECTORS  / 512)       // 4 data blk per bank, 2 real blocks if overwriting
//#define NUM_LSECTORS    (NUM_PSECTORS  / 1024)    // 2 data blk per bank, 1 real blocks if overwriting
